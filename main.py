...
class Comment(db.Model):
    ...
    post: Mapped["BlogPost"] = relationship("BlogPost", back_populates="post_comments")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash('Your email was not found in the database, check your email and try again!')
            redirect(url_for('login'))
...
def get_all_posts():
    ...
    return render_template("posts-archive.html", all_posts=posts, current_user=current_user)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            txt = form.comment.data
            new_comment = Comment(text=txt, comment_author=current_user, post=requested_post)
